...
abstract class BaseEndpoint
{
     ...
     */
    protected function restGetOne(int $id, array $filters): BaseResource
    {
     ...
     * @throws \JsonException|\ReflectionException
     */
    protected function restGetAll(?int $from = null, ?int $limit = null, array $filters = []): BaseResourceCollection
    {
        $this->setCurrentMethod(self::GET)->setCurrentDate();
     ...
     */
    protected function restPost(PostData $resource): BaseResource
    {
        $this->setCurrentMethod(self::POST)->setCurrentDate();

        $uri = $this->getUri();
        $data = $this->checkForEntities($resource->getPostData());
    ...
    private function entityToObject(BaseEntity $data): \stdClass
    {
        ...
        foreach (array_keys($data::$entities) as $property) {
     ...
     * @throws \JsonException
     */
    protected function restPatch(PatchData $resource): void
    {
        $this->setCurrentMethod(self::PATCH)->setCurrentDate();

        $uri = $this->getUri() . '/' . $resource->getId();

        $headers = [
            'headers' => [
                'x-public' => $this->client->apiCredentials->getPublic(),
                'x-hash' => $this->getHash($uri, $resource->getPatchData()),
                'x-date' => $this->getCurrentDate(),
                'accept' => $this->getAcceptHeader(),
                'accept-language' => $this->getAcceptLanguage(),
            ],
            'json' => $resource->getPatchData(),
        ];
        $this->doCall($uri, $headers);
    }

    /**
     * @param PutData $resource
     *
     * @return void
     *
     * @throws GuzzleException
     ...
     * @throws \JsonException
     */
    protected function restPut(PutData $resource): void
    {
        ...
        $headers = [
            'headers' => [
                'x-public' => $this->client->apiCredentials->getPublic(),
                'x-hash' => $this->getHash($uri, $resource->getPutData()),
                'x-date' => $this->getCurrentDate(),
                'accept' => $this->getAcceptHeader(),
            ],
            'json' => $resource->getPutData(),
        ];
        $this->doCall($uri, $headers);
    }
     ...
     * @throws \JsonException
     */
    protected function restDelete(int $id): void
    {
     ...
     * @throws InvalidHashOnResult
     */
    protected function validateResponse(\Psr\Http\Message\ResponseInterface $res, string $uri): void
    {
        $dataToHash = [
