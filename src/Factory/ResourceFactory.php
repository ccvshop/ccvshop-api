...
class ResourceFactory
{
    ...
    {
        $properties = get_object_vars($apiResult);
        foreach ($properties as $property => $value) {
            if (! property_exists($resource, $property)) {
                continue;
            }

            if (! empty($value) && ! empty($resource->dates) && in_array($property, $resource->dates, true)) {
                ...
                $entity = self::objectToEntityClass($value, $resource->entities[$property]);
     ...
     *
     * @return BaseEntity|BaseEntityCollection
     *
     * @throws \ReflectionException
     */
    private static function objectToEntityClass($value, string $entityClass)
    {
        ...
        if ($entity instanceof BaseEntityCollection && is_array($value)) {
            // Each entity collection property has its own entity class.
            foreach ($value as $element) {
                $elementClass = self::objectToEntityClass($element, $entity::$entityClass);
                $entity->addItem($elementClass);
            }
        } elseif ($entity instanceof BaseEntity && is_object($value)) {
            $entity = self::assignValueToEntity($entity, $value);
        }
    ...
    private static function assignValueToEntity(BaseEntity $entity, \stdClass $value): BaseEntity
    {
        foreach ($entity::$entities as $property => $class) {
            ...
            if (isset($value->{$property})) {
                $entity->{$property} = self::objectToEntityClass($value->{$property}, $class);
            }
        }
    ...
    {
        return new ParentResource($resource->getEndpoint()->getResourcePath(), $resource->getId());
    }
